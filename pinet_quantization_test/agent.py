...
class Agent(nn.Module):
    ...
    def predict_lanes_test(self, inputs):
        ...
        outputs, features = self.lane_val(inputs)
    ...
    def evaluate_mode(self):
        self.lane_detection_network.to("cpu")
        self.lane_val = torch.quantization.convert(self.lane_detection_network.eval(), inplace=False)
        self.lane_val.eval()
        traced_script_module = torch.jit.trace(self.lane_val, torch.rand(self.p.batch_size, 3, self.p.x_size, self.p.y_size))
        traced_script_module.save(self.p.save_path+'lane_detection_network.pt')
    ...
    def load_weights(self, epoch, loss):
        ...
        self.lane_detection_network.fuse_model()
        self.lane_detection_network.qconfig = torch.quantization.get_default_qat_qconfig('qnnpack')
        self.lane_detection_network = torch.quantization.prepare_qat(self.lane_detection_network, inplace=False)   
    ...
    def d_observer(self):
        self.lane_detection_network.apply(torch.quantization.disable_observer)
    def freeze_bn(self):
        self.lane_detection_network.apply(torch.nn.intrinsic.qat.freeze_bn_stats)
