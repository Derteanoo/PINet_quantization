...
def draw_point_ori(x, y, image, ratio_w, ratio_h,h_crop=0):
    color_index = 0
    for id in range(len(x)):
        ...
        x_l = x[id]
        #print(x_l)
        x_list = [int(x / ratio_w) for x in x_l]
        
        y_l = y[id]
        y_list = [int(y / ratio_h)+h_crop for y in y_l]
...
def choose_color(x, image_w):
    ...
    if average_x > 0 and average_x < image_w / 24:
        ...
        color = p.color[10]
    return color
def curve_fit(image, x_list, y_list):
    x = np.array(x_list)
    y = np.array(y_list)
    ...
    color = choose_color(x,image.shape[1]) # 根据网格选择颜色
...
def draw_lines_ori(x, y, image, ratio_w, ratio_h):
    ...
    for id in range(len(x)):
        ...
        y_l = y[id]
        y_list = [int(y / ratio_h) for y in y_l]
