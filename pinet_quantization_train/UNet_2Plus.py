...
class UNet_2Plus(nn.Module):
    def __init__(self, in_channels=32, n_classes=32, feature_scale=4, is_deconv=False, is_batchnorm=True):
    ...
    def fuse_model(self):
        ...
        for i, m in enumerate(modules_list):
            if type(m) == nn.Sequential and len(m)==3:
