...
class Generator(object):
    ...
    def __init__(self):
        ...
        self.train_data_all = []
        with open(self.p.train_root_url+"crop_train.json") as f:
            while True:
                line = f.readline()
                if not line:
                    break
                jsonString = json.loads(line)
                self.train_data_all.append(jsonString)
        random.shuffle(self.train_data_all)
        self.size_train = len(self.train_data_all)
        print("size_train: ",self.size_train)

        # load test set
        self.test_data = []
        with open(self.p.test_root_url+'crop_val.json') as f:
            while True:
                line = f.readline()
                if not line:
                    break
        ...
        print("size_test: ",self.size_test)
    #################################################################################################################
    ...
    def Generate_Test(self): 
        cuts = [(b, min(b + 1, self.size_test)) for b in range(0, self.size_test, 1)]
        for start, end in cuts:
